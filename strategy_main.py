...
@st.cache_data
def short_strategy(df, dma_period, pc):
    ...
    final = pd.merge(mon, trade_data[['thu_close', 'thu_date', 'fri_close', 'fri_date', 'trade_flag', 'rank']], on='rank', how='left')
    final['abs_returns'] = round(final['mon_close'] - final['fri_close'], 2) * -1
    final['%_returns'] = round((final['mon_close'] - final['fri_close']) * 100 / final['fri_close'], 2) * -1
    final['gains_flag'] = 0
...
def get_performance(final):
    met_dict = {
        ...
        'total_loss_points': round((final[final['gains_flag'] == 0]['abs_returns'].sum()), 2) * -1,
